...
//IM ON STEP 4 GO THERE WHEN YOU COME BACK
...
class EmojiCollectionViewController: UICollectionViewController {
    ...
    var activeLayout: Layout = .grid {
        didSet{
            if let layout = layout[activeLayout] {
                ...
                collectionView.setCollectionViewLayout(layout, animated: true) { (_) in switch self.activeLayout {
                case .grid:
    ...
    func indexPath(for emoji: Emoji) -> IndexPath? {
        if let sectionIndex = sections.firstIndex (
            where: { $0.title == emoji.sectionTitle }),
